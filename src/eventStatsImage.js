...
function renderTile(x, y, width, height, label, value, valueColor) {
  ...
  return `
    <g>
      ...
      <text x="${x + padding}" y="${y + padding + labelSize * 0.85}" fill="${COLORS.muted}" font-size="${labelSize}" font-weight="500" font-family="system-ui, sans-serif">${escapeXml(label)}</text>
      <text x="${x + width / 2}" y="${valueCenterY + valueSize * 0.35}" text-anchor="middle" fill="${valueColor}" font-size="${valueSize}" font-weight="700" font-family="system-ui, sans-serif">${escapeXml(value)}</text>
    </g>`;
}
...
function renderTimelineChart(box, title, points, color, asArea) {
  ...
  return `
    <g>
      ...
      <text x="${box.x}" y="${box.y + titleSize}" fill="${COLORS.text}" font-size="${titleSize}" font-weight="600" font-family="system-ui, sans-serif">${escapeXml(title)}</text>
      <text x="${box.x + box.width}" y="${box.y + titleSize}" text-anchor="end" fill="${COLORS.muted}" font-size="${axisSize}" font-family="system-ui, sans-serif">max ${maxValue}</text>
      <path d="${path}" fill="${asArea ? color + "33" : "none"}" stroke="${color}" stroke-width="3"/>
      <text x="${box.x}" y="${box.y + box.height - 2}" fill="${COLORS.muted}" font-size="${axisSize}" font-family="system-ui, sans-serif">${escapeXml(firstDate)}</text>
      <text x="${box.x + box.width}" y="${box.y + box.height - 2}" text-anchor="end" fill="${COLORS.muted}" font-size="${axisSize}" font-family="system-ui, sans-serif">${escapeXml(lastDate)}</text>
    </g>`;
}
...
function renderDonut(centerX, centerY, radius, met, notMet) {
  ...
  return `
    <g>
      ...
      <text x="${centerX}" y="${centerY + percentSize * 0.18}" text-anchor="middle" fill="${COLORS.text}" font-size="${percentSize}" font-weight="700" font-family="system-ui, sans-serif">${percentLabel}</text>
      <text x="${centerX}" y="${centerY + percentSize * 0.18 + captionSize + 6}" text-anchor="middle" fill="${COLORS.muted}" font-size="${captionSize}" font-family="system-ui, sans-serif">met goal</text>
    </g>`;
}
...
function renderDonutPanel(box, met, notMet) {
  ...
  return `
    <g>
      <rect x="${box.x - 16}" y="${box.y - 16}" width="${box.width + 32}" height="${box.height + 32}" rx="12" fill="${COLORS.panel}" stroke="${COLORS.panelEdge}"/>
      <text x="${box.x}" y="${box.y + titleSize}" fill="${COLORS.text}" font-size="${titleSize}" font-weight="600" font-family="system-ui, sans-serif">Hour goal split</text>
      ...
      <text x="${legendX + swatch + 10}" y="${donutCenterY - 12}" fill="${COLORS.text}" font-size="${legendSize}" font-family="system-ui, sans-serif">Met (${met})</text>
      <rect x="${legendX}" y="${donutCenterY + 8}" width="${swatch}" height="${swatch}" fill="${COLORS.bad}" rx="3"/>
      <text x="${legendX + swatch + 10}" y="${donutCenterY + swatch + 4}" fill="${COLORS.text}" font-size="${legendSize}" font-family="system-ui, sans-serif">Not met (${notMet})</text>
    </g>`;
}
...
function renderFunnel(box, qualification) {
  ...
  const rows = stages.map((stage, index) => {
    ...
    return `
      <text x="${box.x}" y="${centerY + labelSize * 0.35}" fill="${COLORS.muted}" font-size="${labelSize}" font-family="system-ui, sans-serif">${escapeXml(stage.label)}</text>
      <rect x="${barLeft}" y="${rowY + rowHeight * 0.18}" width="${barWidth.toFixed(1)}" height="${rowHeight * 0.64}" rx="6" fill="${COLORS.funnel[index]}"/>
      <text x="${barLeft + barWidth + 14}" y="${centerY + valueSize * 0.35}" fill="${COLORS.text}" font-size="${valueSize}" font-weight="700" font-family="system-ui, sans-serif">${stage.value}</text>`;
  });
  return `
    <g>
      <rect x="${box.x - 16}" y="${box.y - 16}" width="${box.width + 32}" height="${box.height + 32}" rx="12" fill="${COLORS.panel}" stroke="${COLORS.panelEdge}"/>
      <text x="${box.x}" y="${box.y + titleSize}" fill="${COLORS.text}" font-size="${titleSize}" font-weight="600" font-family="system-ui, sans-serif">Qualification funnel</text>
      ${rows.join("")}
    </g>`;
}
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ${bgRect}
  ${headerIcon}
  <text x="${titleX}" y="${titleSize + 10}" fill="${COLORS.text}" font-size="${titleSize}" font-weight="700" font-family="system-ui, sans-serif">${escapeXml(event.title)}</text>
  <text x="40" y="${titleSize + subtitleSize + 24}" fill="${COLORS.muted}" font-size="${subtitleSize}" font-family="system-ui, sans-serif">${escapeXml(headerSubtitle)}</text>
  <text x="${width - 40}" y="${titleSize + 10}" text-anchor="end" fill="${COLORS.muted}" font-size="${generatedSize}" font-family="system-ui, sans-serif">generated ${escapeXml(stats.generatedAt)}</text>
  ${tiles}
  ${pinnedChart}
  ${dauChart}
  ${donutPanel}
